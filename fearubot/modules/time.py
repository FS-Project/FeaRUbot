...
from fearubot import CMD_HELP, COUNTRY, TZ_NUMBER
...
async def time_func(tdata):
    ...
    if c_name != COUNTRY:
        await tdata.edit(f"Sekarang  **{dtnow}**  di {c_name}({time_zone} zona waktu).")
        return
...
async def date_func(dat):
    ...
    if c_name != COUNTRY:
        await dat.edit(f"Sekarang  **{dtnow}**  di {c_name}({time_zone} zona waktu).")
        return
