...
import fearubot.modules.sql_helper.google_drive_sql as helper
...
from fearubot.modules.aria import aria2, check_metadata
from fearubot.utils import human_to_bytes, humanbytes, progress, time_formatter
from fearubot.utils.exceptions import CancelProcess
...
async def generate_credentials(gdrive):
    ...
    if helper.get_credentials(str(gdrive.from_id)) is not None:
        await gdrive.edit("Berhasil mengotorisasi token..")
    ...
    if G_DRIVE_DATA is not None:
        try:
            ...
            await gdrive.edit(
                "[AUTENTIKASi - ERROR]\n\n"
                "Status : **BAD**\n"
                "Alasan : **G_DRIVE_DATA** tidak valid!"
            )
            return False
    else:
        """ - Only for old user - """
    ...
    async with gdrive.client.conversation(BOTLOG_CHATID) as conv:
        ...
        await gdrive.edit("Kredensial berhasil dibuat...")
...
async def upload(gdrive, service, file_path, file_name, mimeType):
    try:
        await gdrive.edit("Prosesss Unggah...")
    except Exception:
        pass
    body = {
        "nama": file_name,
        "deskripsi": "Berhasil diunggah dari telegram menggunakan FeaRUserbot.",
        "mimeType": mimeType,
    }
    try:
        if parent_Id is not None:
...
async def google_drive_managers(gdrive):
    ...
    await gdrive.edit("Sedang prosess...")
    service = await create_app(gdrive)
    if service is False:
        return None
...
async def google_drive(gdrive):
    ...
    if not value and not gdrive.reply_to_msg_id:
        ...
        await gdrive.edit(
            "`[UNKNOWN - ERROR]`\n\n"
            "Status : **GAGAL**\n"
            "Alasan : Apa yang akan diunggah file atau pesan/media yang dibalas tidak ada."
        )
        return None
    service = await create_app(gdrive)
    if service is False:
    ...
    if isfile(value):
        ...
        if not uri and not gdrive.reply_to_msg_id:
            await gdrive.edit(
                ...
                "nilai file/folder, misalnya `.gd <namafile1> <namafile2>` "
...
async def set_upload_folder(gdrive):
        ...
        if G_DRIVE_FOLDER_ID is not None:
            ...
            try:
                ...
                await gdrive.edit(
                    "`[FOLDER - SET]`\n\n"
                    "Status : **OK**"
                    " - `G_DRIVE_FOLDER_ID` kosong, akan menggunakan folder root."
                )
                return None
    ...
    try:
        ...
        if "uc?id=" in ext_id:
            await gdrive.edit(
                "`[URL - ERROR]`\n\n" "Status : **BAD** - Folder URL tidak valid."
            )
            return None
        try:
